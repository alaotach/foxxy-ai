// Supertonic Text-to-Speech Helper for Foxy AI Extension
// Provides voice responses using Supertonic TTS
// NOTE: This requires ONNX Runtime Web to be available globally as 'ort'
// Download from: https://cdn.jsdelivr.net/npm/onnxruntime-web@1.17.0/dist/ort.min.js
// and include it in the extension or use the bundled version

// ONNX Runtime should be loaded via script tag in HTML, not imported here due to CSP
...
function getOrt() {
    ...
    throw new Error('ONNX Runtime not loaded. Please download ort.min.js to extension/libs/');
}
...
export class TextToSpeech {
    ...
    async _infer(textList, langList, style, totalStep, speed = 1.05, progressCallback = null) {
        ...
        const textIdsTensor = new (getOrt().Tensor)('int64', textIdsFlat, textIdsShape);
        
        const textMaskFlat = new Float32Array(textMask.flat(2));
        const textMaskShape = [bsz, 1, textMask[0][0].length];
        const textMaskTensor = new (getOrt().Tensor)('float32', textMaskFlat, textMaskShape);
        ...
        const latentMaskTensor = new (getOrt().Tensor)('float32', latentMaskFlat, latentMaskShape);
        
        const totalStepArray = new Float32Array(bsz).fill(totalStep);
        const totalStepTensor = new (getOrt().Tensor)('float32', totalStepArray, [bsz]);
        ...
        for (let step = 0; step < totalStep; step++) {
            ...
            const currentStepTensor = new (getOrt().Tensor)('float32', currentStepArray, [bsz]);
            
            const xtFlat = new Float32Array(xt.flat(2));
            const xtShape = [bsz, xt[0].length, xt[0][0].length];
            const xtTensor = new (getOrt().Tensor)('float32', xtFlat, xtShape);
        ...
        const finalXtTensor = new (getOrt().Tensor)('float32', finalXtFlat, finalXtShape);
 ...
 */
export async function loadVoiceStyle(voiceStylePath, verbose = false) {
    ...
    const ttlTensor = new (getOrt().Tensor)('float32', ttlData, voiceStyle.style_ttl.dims);
    const dpTensor = new (getOrt().Tensor)('float32', dpData, voiceStyle.style_dp.dims);
    
    if (verbose) {
        console.log('Loaded voice style');
    }
...
export async function loadOnnx(onnxPath, options) {
    const session = await getOrt().InferenceSession.create(onnxPath, options);
    return session;
}
